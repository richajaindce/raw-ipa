    ...
    F: Field,
{
    ...
    for (bit_num, one_slice) in sort_keys.iter().enumerate().skip(1) {
        ...
        malicious_validator = MaliciousValidator::new(sh_ctx.narrow(&Sort(bit_num.try_into().unwrap())));
        m_ctx_bit = malicious_validator.context();
        let upgraded_sort_keys = m_ctx_bit
            .upgrade(one_slice.clone())
            .await?;
        ...
        let next_few_bits_sorted_by_less_significant_bits = secureapplyinv_multi(
            m_ctx_bit.narrow(&MultiApplyInv(bit_num.try_into().unwrap())),
            upgraded_sort_keys,
            (randoms_for_shuffle0, randoms_for_shuffle1),
            revealed,
        )
...
mod tests {
    ...
    use crate::test_fixture::{MaskedMatchKey, Runner, join3};
    use crate::{
        ff::{Field, Fp31},
    ...
    pub async fn malicious_sort_in_semi_honest() {
        ...
        let [(v0, result0), (v1, result1), (v2, result2)] = world
            .semi_honest(match_keys.clone(), |ctx: SemiHonestContext<Fp31>, mk_shares| async move {
                let local_lists = convert_all_bits_local(ctx.role(), &mk_shares, MaskedMatchKey::BITS);
                let converted_shares =
                    convert_all_bits(&ctx, local_lists, MaskedMatchKey::BITS, NUM_MULTI_BITS).await.unwrap();
